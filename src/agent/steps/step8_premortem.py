...
class Step8Premortem(BaseStep):
    ...
    def _build_system_prompt(self, state: AnalysisState | None = None) -> str:
        ...
        return (
            ...
            "STRUCTURAL_UNHEDGEABLE should be RARE, reserved for the minority of cases "
            "where the primary risk alone would justify rejection even if governance, "
            "financials, and valuation were all excellent. Being ABLE to construct a "
            "plausible severe scenario is not sufficient — nearly every company carries "
            "some headwind (competition, regulation, technology shift, cyclicality); "
            "that alone does not make it structural. Ask: is this exposure fundamentally "
            "different in kind from the ordinary competitive and cyclical risk every "
            "business in this sector carries, or is it the same kind of risk just "
            "described vividly? A concrete, specifically-named threat (a named "
            "competitor, a named technology) is NOT automatically more severe than an "
            "abstract one (\"commodity cycles\", \"the market\") — judge the underlying "
            "economic exposure, not how vividly the narrative reads. Default to "
            "CYCLICAL_MANAGEABLE unless the evidence clearly crosses this bar.\n\n"
            + categories
            + "\n\nRULES:\n"
